...
var HOURS = {
  ...
  days: {                             // source: Restaurantji listing, updated 2026-04-27
    0: [8, 26],                       // Sunday     8 am – 2 am
    1: [10, 26],                      // Monday    10 am – 2 am
    2: [10, 26],                      // Tuesday   10 am – 2 am
    3: [10, 26],                      // Wednesday 10 am – 2 am
    4: [10, 26],                      // Thursday  10 am – 2 am
    5: [10, 26],                      // Friday    10 am – 2 am
    6: [8, 26]                        // Saturday   8 am – 2 am
  }
};
...
(function(){
  ...
  /* ---------- SIGNATURE GADGET ----------
     PICK A GAME — the four things they actually have out back and inside:
     a pool table, the shuffleboard, the ping-pong table and the stage.
     Their own roadside sign sells the place as "eats | drinks | games" and
     "entertainment | beer garden", so the games ARE the signature.
     Toddler law: four labelled pills, one tap, the real photo swaps in
     right above them. No modes, no cycling, no legend. Tap the one that's
     already on and nothing breaks — it just stays on. */

  var pills=$$('.game-pills button'), shots=$$('.game-shot img'),
      capName=$('#gameName'), capLine=$('#gameLine');
  if(pills.length && shots.length && capName && capLine){
    function pickGame(key){
      pills.forEach(function(b){b.setAttribute('aria-pressed', b.dataset.game===key ? 'true':'false')});
      shots.forEach(function(im){im.classList.toggle('on', im.dataset.game===key)});
      var b=pills.filter(function(x){return x.dataset.game===key})[0];
      if(b){capName.textContent=b.dataset.name; capLine.textContent=b.dataset.line}
    }
    pills.forEach(function(b){
      b.addEventListener('click',function(){pickGame(b.dataset.game)});
    });
    pickGame(pills[0].dataset.game);
  }
