...
var HOURS = {
  ...
  days: {
    ...
    4: [10.5, 21],                    // Thursday
    5: [10.5, 21],                    // Friday
    6: [10.5, 20.5]                   // Saturday    10:30 AM - 8:30 PM
  }
};
...
(function(){
  ...
  /* ---------- TITLE CARDS UNROLL (the board) ----------
     Every headline in #menu is itself the tap target; the chevron flips up.
     One state, no cycling. */
  ...
  /* ---------- SIGNATURE GADGET — ORDER BY NUMBER ----------
     Five of their own board plates. Tap one, its real counter code comes up
     big with what it is. Tap the same one again and it clears. Nothing to
     read first, nothing to add up, no prices restated anywhere. */
  (function(){
    var row = $('#plateRow'), shown = $('#plateShown'),
        chip = $('#plateChip'), name = $('#plateName');
    if(!row || !shown) return;
    var REST = 'Tap a plate.';
    function clear(){
      $$('.plate', row).forEach(function(b){ b.setAttribute('aria-pressed','false') });
      shown.setAttribute('data-on','0');
      chip.textContent = '?';
      name.textContent = REST;
    }
    $$('.plate', row).forEach(function(btn){
      btn.addEventListener('click', function(){
        ...
        chip.textContent = btn.getAttribute('data-code');
