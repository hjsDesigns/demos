...
var HOURS = {
  ...
  days: {
    0: [11, 19],                      // Sunday    11:00 AM – 7:00 PM
...
/* SOURCE OF TRUTH: the sign the wagon posts in its own order window —
   "OPEN 11-8 DAILY / SUNDAY 11-7 / Drive Thru Only Please"
   (harvest-0906/g26.jpg, shot again in harvest-0906/g50.jpg).
   Restaurantji (updated Aug 23, 2026) lists 11 AM – 8 PM on all seven days;
   where the two disagree — Sunday — the business's own posted sign wins.
   Google Maps agrees on the 11 AM open. Labor Day / holiday hours are NOT
   published anywhere we could reach: regular Monday hours are shown. */
...
(function(){
  ...
  /* ---------- TITLE CARDS UNROLL (ARROW LAW: a downward chevron that flips up) ---------- */
  ...
  /* ---------- SIGNATURE GADGET — the shake machine ----------
     Their board prints exactly five milkshake flavours and nothing else about
     them. Tap one: the cup fills in that colour and the name lands under it.
     Tap the same one again: the cup empties. One gesture, one visible payoff,
     no numbers to read and no sentence narrating the choice back. */
  (function(){
    var cup=$('#cup'), fill=$('#cupFill'), name=$('#shakeName'), bank=$('#flavors');
    if(!cup||!fill||!name||!bank) return;
    var EMPTY='Empty cup', current=null;
    function clear(){
      current=null;
      cup.classList.remove('is-full');
      name.textContent=EMPTY; name.classList.add('empty');
      $$('.flav',bank).forEach(function(b){b.setAttribute('aria-pressed','false')});
    }
    $$('.flav',bank).forEach(function(btn){
      btn.addEventListener('click',function(){
        if(current===btn){ clear(); return; }
        current=btn;
        $$('.flav',bank).forEach(function(b){b.setAttribute('aria-pressed', b===btn ? 'true':'false')});
        fill.style.background=getComputedStyle(btn).getPropertyValue('--sw').trim();
        cup.classList.add('is-full');
        name.textContent=btn.getAttribute('data-n');
