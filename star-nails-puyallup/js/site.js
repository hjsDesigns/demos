...
var HOURS = {
  ...
  days: {
    0: [11, 17],                      // Sunday     11:00 AM - 5:00 PM
  ...
  // Source: their own Fresha booking page + Google listing (both agree, 2026-09-06).
  // Their printed business card still reads "Mon-Sat 9:30am-7pm" — older print run;
  // flagged for the owner in HANDOFF-CODEX.md. No holiday hours are published
  // anywhere, so Labor Day shows the regular Monday row.
};
...
(function(){
  ...
  /* ---------- SIGNATURE GADGET — PICK A SHAPE ----------
     Four labelled targets, one real photo of theirs each. Tap = instant swap,
     always exactly one on, no readback sentence, no hidden modes.
     Photos are preloaded on first interaction so the swap never flashes. */
  var pills=$$('.shape-pill'), sImg=$('#shapeImg'), sCap=$('#shapeCap'), preloaded=false;
  function preload(){
    if(preloaded)return; preloaded=true;
    pills.forEach(function(b){var i=new Image(); i.src=b.getAttribute('data-img')});
  }
  if(pills.length && sImg && sCap){
    pills.forEach(function(b){
      b.addEventListener('mouseenter',preload,{once:true});
      b.addEventListener('focus',preload,{once:true});
      b.addEventListener('click',function(){
        preload();
        if(b.classList.contains('is-on'))return;
        pills.forEach(function(o){o.classList.toggle('is-on',o===b);o.setAttribute('aria-pressed',o===b?'true':'false')});
        var src=b.getAttribute('data-img');
        sImg.classList.remove('is-on');
        var swap=function(){
          sImg.src=src;
          sImg.alt=b.getAttribute('data-alt')||'';
          sCap.textContent=b.getAttribute('data-cap');
          requestAnimationFrame(function(){sImg.classList.add('is-on')});
        };
