...
(function(){
  ...
  var menuButtons=$$('.menu-tabs button'), menuPanels=$$('.menu-panel'), status=$('#menu-status');
  function showMenu(key){
    menuButtons.forEach(function(button){button.setAttribute('aria-pressed',String(button.dataset.menu===key))});
    menuPanels.forEach(function(panel){panel.hidden=panel.id!=='menu-'+key});
    var button=menuButtons.find(function(item){return item.dataset.menu===key});
    status.textContent=button.textContent+' menu shown';
  }
  menuButtons.forEach(function(button){button.addEventListener('click',function(){showMenu(button.dataset.menu)})});
  showMenu('breakfast');
