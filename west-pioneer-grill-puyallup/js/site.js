...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
/* WEST PIONEER GRILL — 109 W Pioneer Ave, Puyallup WA.
   SOURCES (two, and they disagree on Monday — see HANDOFF-CODEX.md):
     · Google Maps, read 2026-09-06 night: "Opens 8 AM Mon".
     · Restaurantji listing (the only complete 7-day set we could source):
       Mon–Wed 10–7, Thu–Fri 10–8, Sat 9–8, Sun 9–6.
   Google is the fresher, owner-maintained source, so Monday's OPEN is 8:00.
   The rest of the week is the Restaurantji set. THE WHOLE TABLE IS ON THE
   ASK-CLIENT LIST — confirm all seven rows with the owner before go-live. */
var HOURS = {
  tz: 'America/Los_Angeles',          // Pacific, wherever the viewer is
  days: {
    0: [9, 18],                       // Sunday      9:00 am – 6:00 pm
    1: [8, 19],                       // Monday      8:00 am – 7:00 pm  (open per Google Maps)
    2: [10, 19],                      // Tuesday    10:00 am – 7:00 pm
    3: [10, 19],                      // Wednesday  10:00 am – 7:00 pm
    4: [10, 20],                      // Thursday   10:00 am – 8:00 pm
    5: [10, 20],                      // Friday     10:00 am – 8:00 pm
    6: [9, 20]                        // Saturday    9:00 am – 8:00 pm
  }
};
...
(function(){
  ...
  /* ---------- SIGNATURE GADGET — THE SHAKE BOARD ----------
     ...
     tap pours it into the glass and writes the name underneath; tapping the
     same flavor again empties the glass. No modes, no legend, no instructions.
     The swatch colour rides on each button's data-c attribute in the markup. */
  (function shakeBoard(){
    var board=$('#shakeBoard'), glass=$('#glassWrap'), label=$('#glassLabel');
    if(!board||!glass||!label) return;
    var buttons=$$('.fl',board), current=null;
    var IDLE='Tap a flavor';

    // paint each flavor's own dot from its data-c so the board reads at a glance
    buttons.forEach(function(b){ b.style.setProperty('--sw', b.getAttribute('data-c')); });

    function clear(){
      current=null;
      buttons.forEach(function(b){b.classList.remove('on');b.setAttribute('aria-pressed','false')});
      glass.classList.remove('is-full');
      glass.style.removeProperty('--shake');
      label.textContent=IDLE;
    }
    function pour(btn){
      if(current===btn){clear();return}
      current=btn;
      buttons.forEach(function(b){var on=b===btn;b.classList.toggle('on',on);b.setAttribute('aria-pressed',on?'true':'false')});
      glass.style.setProperty('--shake', btn.getAttribute('data-c'));
      glass.classList.add('is-full');
