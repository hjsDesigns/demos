...
var HOURS = {
  ...
  days: {
    0: null,                          // Sunday    — closed (their printed menu: "Sunday: Close")
    1: [12, 20],                      // Monday    — 12:00 pm – 8:00 pm
    2: [12, 20],                      // Tuesday
    3: [12, 20],                      // Wednesday
    4: [12, 20],                      // Thursday
    5: [12, 20],                      // Friday
...
(function(){
  ...
  /* ---------- TITLE CARDS UNROLL — the heading is the tap target ---------- */
  ...
  /* ---------- SIGNATURE — THE FOUR SEASONS TABLE ----------
     Their four family-style dinners, read straight off the printed menu
     (images/menu-sheet.jpg, January 2024). Nothing here is invented, and the
     dish counts really do climb 5 -> 6 -> 7 -> 8 through the year. */
  var SEASONS={
    spring:{price:'$19.50',dishes:['BBQ Pork','Pork & Chicken Chow Mein','Deep Fried Prawns','Sweet & Sour Pork','Pork Fried Rice']},
    summer:{price:'$21.50',dishes:['BBQ Pork','Pork & Chicken Chow Mein','Almond Chicken','Deep Fried Prawns','Sweet & Sour Pork','Pork Fried Rice']},
    autumn:{price:'$22.50',dishes:['BBQ Pork','Egg Rolls','Sub Gum Almond Pork & Chicken Chow Mein','Almond Chicken','Deep Fried Prawns','Sweet & Sour Pork','Pork Fried Rice']},
    winter:{price:'$23.50',dishes:['Won Ton Soup','BBQ Pork','Egg Rolls','Breaded Sesame Fried Chicken','Sweet & Sour Pork','Deep Fried Prawns','Beef Chow Yuk','Pork Fried Rice']}
  };
  var fsWrap=$('.fs'), fsChips=$$('.fs-chip'), fsDishes=$('#fsDishes'),
      fsPrice=$('#fsPrice'), fsCount=$('#fsCount'), fsLive=$('#fsLive'), fsTable=$('#fsTable');
  if(fsWrap && fsDishes){
    var fsCurrent=null;
    var reduce=window.matchMedia('(prefers-reduced-motion:reduce)').matches;
    function fsClear(){
      fsCurrent=null;
      fsWrap.classList.remove('is-set');
      fsWrap.removeAttribute('data-season');
      fsDishes.innerHTML='';
      fsPrice.textContent=''; fsCount.textContent='';
      fsChips.forEach(function(c){c.setAttribute('aria-pressed','false')});
      if(fsLive) fsLive.textContent='Table cleared.';
    }
    function fsSet(key){
      var s=SEASONS[key]; if(!s) return;
      fsCurrent=key;
      fsWrap.classList.add('is-set');
      fsWrap.setAttribute('data-season',key);
      fsDishes.innerHTML='';
      s.dishes.forEach(function(name,i){
        var li=document.createElement('li');
        li.innerHTML='<span class="fs-plate" aria-hidden="true"></span><b></b>';
        li.querySelector('b').textContent=name;
        li.style.animationDelay=(reduce?0:(i*90))+'ms';
        fsDishes.appendChild(li);
      });
      fsPrice.textContent=s.price;
      fsCount.textContent=s.dishes.length+' dishes';
      fsChips.forEach(function(c){c.setAttribute('aria-pressed', c.getAttribute('data-season')===key?'true':'false')});
      if(fsLive) fsLive.textContent=s.dishes.length+' dishes, '+s.price+' per dinner.';
    }
    fsChips.forEach(function(chip){
      chip.addEventListener('click',function(){
        var key=chip.getAttribute('data-season');
        if(fsCurrent===key){fsClear();return}
        fsSet(key);
      });
    });
    if(fsTable){fsTable.addEventListener('click',function(){ if(fsCurrent) fsClear(); });}
  }
