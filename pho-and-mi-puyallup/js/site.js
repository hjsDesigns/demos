/* ============================================================
   PHO&MI — Puyallup, WA — site.js
   Nav toggle · live open/closed clock (Pacific) · scroll-reveal ·
   count-up · contact form · title-cards-unroll · THE PLATE (side plate)
   ============================================================ */

/* ------------------------------------------------------------
   HOURS — read straight off their OWN online-ordering system,
   phomi.menu11.com/puyallup/order (the "Hours" table on that page,
   pulled 2026-09-06). Google's place page agrees: "Opens 11 AM Mon".
     Monday – Saturday   11:00 AM – 9:00 PM
     Sunday              CLOSED
   0 = Sunday … 6 = Saturday. Decimal 24h (21 = 9 PM).
   ------------------------------------------------------------ */
var HOURS = {
  tz: 'America/Los_Angeles',
  days: {
    0: null,        // Sunday    closed
    1: [11, 21],    // Monday    11:00 AM – 9:00 PM
    2: [11, 21],    // Tuesday
    3: [11, 21],    // Wednesday
    4: [11, 21],    // Thursday
    5: [11, 21],    // Friday
...
/* No holiday closures published by the restaurant — nothing invented here. */
...
(function(){
  ...
  var io=new IntersectionObserver(function(entries){
    entries.forEach(function(en){
      ...
      $$('.count',en.target).forEach(function(c){
        ...
        var to=parseFloat(c.getAttribute('data-to')), from=parseFloat(c.getAttribute('data-from')||'0'), dec=parseInt(c.getAttribute('data-dec')||'0',10), t0=null;
        var show=function(n){c.textContent=dec?n.toFixed(dec):Math.round(n).toLocaleString('en-US')};
        if(REDUCE()){show(to);return}
        function step(ts){if(!t0)t0=ts;var k=Math.min(1,(ts-t0)/1400);var e=1-Math.pow(1-k,3);show(from+(to-from)*e);if(k<1)requestAnimationFrame(step)}
  ...
  /* ---------- SIGNATURE — THE PLATE ON THE SIDE -------------------
     Their pho lands with a side plate: Thai basil, bean sprouts, lime,
     jalapeño (their own photo, further down this section), and the
     sauces are at the counter. Tap a thing, it drops into the bowl and
     the broth rings. Tap it again, it comes back out. Nothing to read,
     nothing to type, no score. */
  var bowl=$('#bowl');
  if(bowl){
    var picks=$$('.pick'), ring=$('#bowlRing');
    function pulse(){
    ...
    picks.forEach(function(btn){
      ...
      btn.addEventListener('click',function(){
        ...
        if(drop) drop.classList.toggle('in',!on);
        if(!on) pulse();
      });
    });
    /* bubbles of steam, positioned once */
    $$('.steam i',bowl).forEach(function(b,i){
      b.style.left=(16+i*17)+'%';
      b.style.animationDelay=(i*1.15)+'s';
    });
  }
