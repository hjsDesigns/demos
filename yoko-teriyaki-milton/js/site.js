...
var HOURS = {
  ...
  days: {
    0: null,                          // Sunday    — closed (Google listing)
    1: [11, 21],                      // Monday      11 AM – 9 PM
    2: [11, 21],                      // Tuesday     11 AM – 9 PM
    3: [11, 21],                      // Wednesday   11 AM – 9 PM
    4: [11, 21],                      // Thursday    11 AM – 9 PM
    5: [11, 21],                      // Friday      11 AM – 9 PM
    6: [11, 21]                       // Saturday    11 AM – 9 PM
  }
};
...
(function(){
  ...
  /* ---------- SIGNATURE GADGET ----------
     Per-client interactive code goes below this line. */

  /* ---------- SIGNATURE GADGET — "Point at it" ----------
     Four real plates from this kitchen. One tap shows it, tapping the same
     one again clears it. No numbers, no modes, nothing to read first. */
  (function(){
    var picker=$('#picker'); if(!picker) return;
    var shots=$$('.pick-window img',picker), btns=$$('.pick',picker), nameEl=$('#pickName');
    function clear(){
      shots.forEach(function(im){im.classList.remove('on')});
      btns.forEach(function(b){b.setAttribute('aria-pressed','false')});
      picker.removeAttribute('data-picked');
    ...
    function show(b){
      ...
      shots.forEach(function(im){im.classList.toggle('on',im.getAttribute('data-i')===i)});
      picker.setAttribute('data-picked',i);
      if(nameEl) nameEl.textContent=b.getAttribute('data-name');
    }
    if(btns[0]) show(btns[0]);          // never open on an empty frame
    btns.forEach(function(b){
      b.addEventListener('click',function(){
        if(b.getAttribute('aria-pressed')==='true'){clear();return}
        show(b);
      });
    });
  })();

  /* ---------- menu title cards: tap the heading, the list rolls out ---------- */
