...
var HOURS = {
  ...
  days: {                             // Google listing + the Sumner directory agree
    0: [11, 16],                      // Sunday     11:00 AM – 4:00 PM
    1: [10, 17],                      // Monday     10:00 AM – 5:00 PM
    2: [10, 17],                      // Tuesday
    3: [10, 17],                      // Wednesday
    4: [10, 17],                      // Thursday
    5: [10, 17],                      // Friday
...
(function(){
  ...
  /* ---------- SIGNATURE GADGET ----------
     Per-client interactive code goes below this line. Keep it inside this IIFE
     so it can use $ / $$ / pacificNow. */

  /* ---------- SIGNATURE GADGET — OFF THE RACK ----------
     Five real things hang on the rail. One tap drops that piece into the frame
     above; tapping the same hanger again puts it back and clears the frame.
     Object + label, nothing narrated back, no numbers to read, no state to
     decode. TODDLER LAW. */
  (function(){
    var shot=$('#rackShot'), img=$('#rackImg'), label=$('#rackLabel');
    var hangers=$$('.rail .hanger');
    if(!shot||!img||!label||!hangers.length) return;

    function clear(){
      hangers.forEach(function(h){h.setAttribute('aria-pressed','false')});
      shot.classList.remove('has-pick');
    ...
    function pick(btn){
      hangers.forEach(function(h){h.setAttribute('aria-pressed', h===btn?'true':'false')});
      img.hidden=false;
      img.src=btn.getAttribute('data-img');
      img.alt=btn.getAttribute('data-alt')||'';
      label.textContent=btn.getAttribute('data-label')||'';
      shot.classList.add('has-pick');
      // let the new src paint before the fade so it never cross-blends two photos
      requestAnimationFrame(function(){requestAnimationFrame(function(){
        img.classList.add('show'); label.classList.add('show');
      })});
    }
    hangers.forEach(function(btn){
      btn.addEventListener('click',function(){
        if(btn.getAttribute('aria-pressed')==='true'){clear()}else{
          img.classList.remove('show'); label.classList.remove('show'); pick(btn);
        }
      });
    });
    // preload so the first tap is instant
    hangers.forEach(function(b){var i=new Image(); i.src=b.getAttribute('data-img')});
  })();

  /* ---------- TITLE CARDS UNROLL ----------
     The headline is the tap target; the chevron flips up (ARROW LAW). */
  (function(){
    $$('.board .unroll').forEach(function(btn){
