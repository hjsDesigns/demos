/* ============================================================
   ...
   count-up · contact form · title-cards-unroll · THE TOT BOARD
   ============================================================ */

/* ------------------------------------------------------------
   HOURS — from Restaurantji's Bumpy's page (listing updated
   Aug 17, 2026), the only 7-day table that could actually be
   loaded and read. Its Sunday close agrees with the live Google
   profile (Sun, closes 12 AM, checked 2026-09-06).
     Mon–Thu  9:00 AM – 12:00 AM
     Fri–Sat  9:00 AM –  1:00 AM
     Sun      9:00 AM – 12:00 AM
   Yelp (9am–11pm) and the Puyallup Main Street Association
   (10am–11:30pm) publish different closes; both are Cloudflare-
   walled and could not be verified — see HANDOFF-CODEX.md.
   0 = Sunday … 6 = Saturday. Decimal 24h. 24 = midnight,
   25 = 1 AM the next morning, so the clock keeps the bar open
   past midnight on Friday and Saturday.
   ------------------------------------------------------------ */
var HOURS = {
  tz: 'America/Los_Angeles',
  days: {
    0: [9, 24],                       // Sunday     9:00 AM – 12:00 AM
    1: [9, 24],                       // Monday     9:00 AM – 12:00 AM
    2: [9, 24],                       // Tuesday
    3: [9, 24],                       // Wednesday
    4: [9, 24],                       // Thursday
    5: [9, 25],                       // Friday     9:00 AM –  1:00 AM
    6: [9, 25]                        // Saturday   9:00 AM –  1:00 AM
  }
};

/* No holiday hours are published anywhere reachable for this bar —
   Labor Day included. Regular hours stand; see the handoff. */
...
(function(){
  ...
  /* ---------- SIGNATURE GADGET — THE TOT BOARD ---------------------
     Four name plates, each carrying its real wording and price off the
     pub's own printed menu. One tap lights that plate like the neon out
     front and drops the item into the readout. Tap the lit one again and
     it goes out. Nothing is invented, nothing is won, no state to decode. */
  var read=$('#boardRead');
  if(read){
    var plates=$$('.plate'),
        rName=$('#readName'), rDesc=$('#readDesc'), rPrice=$('#readPrice'),
        REST={name:'Tap a name below.', desc:'Every one of them starts as a basket of tots.', price:''};

    function paint(name,desc,price,lit){
      rName.textContent=name;
      rDesc.textContent=desc;
      rPrice.textContent=price;
      read.classList.toggle('is-set',!!lit);
    }
    function clear(){
      plates.forEach(function(p){p.setAttribute('aria-pressed','false')});
      paint(REST.name,REST.desc,REST.price,false);
    }

    plates.forEach(function(p){
      p.addEventListener('click',function(){
        var lit = p.getAttribute('aria-pressed')==='true';
        if(lit){ clear(); return; }                       // tap it again, light off
        plates.forEach(function(o){o.setAttribute('aria-pressed', o===p?'true':'false')});
        read.classList.remove('is-set');                  // restart the price pop cleanly
        void read.offsetWidth;
        paint(p.dataset.name, p.dataset.desc, p.dataset.price, true);
      });
    });
  }
