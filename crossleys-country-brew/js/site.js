...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
/* CROSSLEY'S COUNTRY BREW — hours agree exactly across joe.coffee and
   Restaurantji (both listings pulled 2026-09-06). Mon–Fri 4 am – 6 pm,
   Sat & Sun 6 am – 6 pm. No holiday rule is known; nothing invented. */
var HOURS = {
...
(function(){
  ...
  if(form&&ok){
    ...
    btn.disabled=false;
    if(keyEl&&keyEl.value.trim()){btn.textContent='Send my question';help.textContent='Your message goes directly to the stand.';}
    ...
    form.addEventListener('submit',function(e){
      ...
      if(!key){
        ...
        var body=['Question: '+(fields.get('topic')||''),'Name: '+(fields.get('name')||''),'Phone: '+(fields.get('phone')||''),'Email: '+(fields.get('email')||''),'',fields.get('msg')||''].join('\n');
        var area=$('textarea',note);area.value=body;note.hidden=false;
        function selectNote(){area.focus();area.select();feedback('Your question is ready below. Select and copy it to bring to the window; nothing has been sent.',false);}
        if(navigator.clipboard&&navigator.clipboard.writeText){
          navigator.clipboard.writeText(body).then(function(){feedback('Question copied. Keep it handy for your next visit; nothing has been sent.',false);}).catch(selectNote);
        }else selectNote();
        return;
      }
      var label=btn.textContent;btn.disabled=true;btn.textContent='Sending…';
      var data={};new FormData(form).forEach(function(v,k){data[k]=v});data.subject='Question — Crossley’s Country Brew';
      fetch('https://api.web3forms.com/submit',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify(data)})
        ...
        .then(function(result){if(!result||!result.success)throw new Error('send failed');feedback('Message sent. The stand can reply using the contact details you provided.',false);btn.textContent='Message sent';})
        .catch(function(){btn.disabled=false;btn.textContent=label;feedback('Could not send just now. Please ask at the window instead.',true);});
    });
  }

  /* ---------- TITLE CARDS UNROLL (#menu) ----------
     Each heading IS the tap target; the chevron flips up. Nothing hidden
     behind a second gesture, no cycling states. */
  $$('.unroll').forEach(function(card,index){
    var head=$('.unroll-head',card);
    if(!head)return;
    var body=$('.unroll-body',card);body.id='menu-group-'+index;head.setAttribute('aria-controls',body.id);body.setAttribute('aria-hidden','true');body.inert=true;
    head.addEventListener('click',function(){
      var open=card.classList.toggle('open');
      head.setAttribute('aria-expanded',open?'true':'false');body.setAttribute('aria-hidden',open?'false':'true');body.inert=!open;
    });
  });

  /* ---------- SIGNATURE GADGET — the "Thank y'all" board ----------
     Their own posted offer: a free 16 oz drink, a different trade each
     weekday. One tap on a day, the board changes. Always exactly one day
     selected; today is pre-lit Mon–Fri so it reads right on arrival. */
  (function(){
    var board=$('.thankboard'); if(!board)return;
    var picks=$$('.tb-pick',board), dayEl=$('#tbDay'), whoEl=$('#tbWho');
    var NAMES=['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'];
    var swapT=null;
    function show(btn,animate){
      picks.forEach(function(b){b.classList.toggle('is-on',b===btn);b.setAttribute('aria-pressed',b===btn?'true':'false')});
      var d=parseInt(btn.getAttribute('data-day'),10);
      var who=btn.getAttribute('data-who');
      function paint(){dayEl.textContent=NAMES[d];whoEl.innerHTML=who;}
      if(!animate||window.matchMedia('(prefers-reduced-motion:reduce)').matches){paint();return}
      board.classList.add('swap');
      clearTimeout(swapT);
      swapT=setTimeout(function(){paint();board.classList.remove('swap')},180);
    }
    picks.forEach(function(b){b.addEventListener('click',function(){show(b,true)})});
    // This undated offer is an example of their board, not a live promotion.
    show(picks[0],false);
  })();
