...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
/* MY LIL' CUBE — 402 N Meridian, Puyallup WA.
   Source: the OPENING HOURS card taped inside their own front window
   ("MONDAY: CLOSED / TUE - SAT: 11:00 to 21:00 / SUNDAY: 11:00 to 20:00"),
   read off two of their own storefront photos and confirmed against the
   structured hours on their Restaurantji listing. Both agree exactly. */
var HOURS = {
  tz: 'America/Los_Angeles',          // Pacific, wherever the viewer is
  days: {
    0: [11, 20],                      // Sunday      11:00 AM – 8:00 PM
    1: null,                          // Monday      closed
    2: [11, 21],                      // Tuesday     11:00 AM – 9:00 PM
    3: [11, 21],                      // Wednesday   11:00 AM – 9:00 PM
    4: [11, 21],                      // Thursday    11:00 AM – 9:00 PM
...
(function(){
  ...
  /* ---------- TITLE CARDS UNROLL (8/27 law) ----------
     The headline IS the tap target; the chevron flips up; the real items
     roll out in place. Pure state toggle — no measuring, no timers. */
  ...
  /* ---------- SIGNATURE GADGET — THE CHILI OIL ----------
     One tap target (the jar), one instant payoff (it goes in the bowl),
     tap again clears. No numbers, no readback sentence, no hidden modes.
     Deterministic: the same class flip every time, flecks are fixed
     positions with fixed delays in the markup. */
  (function(){
    var stage = $('#chili'), jar = $('#jarBtn'), hint = $('#jarHint'), cap = $('#bowlCap');
    if(!stage || !jar) return;
    var COPY = {
      off: { cap: 'Tonkotsu ramen, as it comes',  hint: 'Tap the chili oil.' },
      on:  { cap: 'Tonkotsu ramen, with the chili oil', hint: 'Tap it again to take it back off.' }
    };
    function set(on){
      stage.classList.toggle('is-on', on);
      jar.setAttribute('aria-pressed', on ? 'true' : 'false');
      var c = on ? COPY.on : COPY.off;
      if(cap) cap.textContent = c.cap;
      if(hint) hint.textContent = c.hint;
      if(on){
        // restart the fleck animation cleanly on every re-tap
        $$('.flecks i', stage).forEach(function(f){
          f.style.animation = 'none'; void f.offsetWidth; f.style.animation = '';
        });
      }
    }
    jar.addEventListener('click', function(){
      set(jar.getAttribute('aria-pressed') !== 'true');
    });
    set(false);
  })();
