/* ============================================================
   SAPPORO & SUSHI RESTAURANT — site.js
   Nav toggle · live open/closed clock (Pacific) · scroll-reveal ·
   count-up · contact form · title-cards-unroll · SAY IT AT THE BAR
   ============================================================ */

/* ------------------------------------------------------------
   HOURS — their own NetWaiter listing (sapporosushiroll.netwaiter.com/fife,
   read 2026-09-08): Mon–Fri 10:30 AM – 9:00 PM, Sat 11:00 AM – 9:00 PM,
   Sunday closed. The Google panel independently confirms Mon–Fri 10:30–9.
   (Their Menu11 ordering page cuts off at 8:45 PM — that's the last online
   order, not the door.)
...
var HOURS = {
  ...
  days: {
    ...
    1: [10.5, 21],                    // Monday    10:30 AM – 9:00 PM
    2: [10.5, 21],                    // Tuesday
    3: [10.5, 21],                    // Wednesday
    4: [10.5, 21],                    // Thursday
    5: [10.5, 21],                    // Friday
...
/* No holiday rules are published for this restaurant — the page says so in
   the hours note rather than guessing at one. Leave these as pass-throughs. */
...
(function(){
  ...
  /* ---------- SIGNATURE GADGET — SAY IT AT THE BAR -----------------
     Five pieces of nigiri on the counter. Tap one: it lifts onto the plate,
     which says what fish it is and what it costs. Tap the same one again and
     it goes back down. One tap target, one payoff, nothing to read first. */
  var netaRow=$('#neta');
  if(netaRow){
    var netas=$$('.neta',netaRow), shown=$('#netaShown'), en=$('#netaEn'),
        price=$('#netaPrice'), empty=$('#netaEmpty'), nhint=$('#netaHint'), taps=0;
    function clearPlate(){
      netas.forEach(function(b){b.setAttribute('aria-pressed','false')});
      shown.setAttribute('data-on','0');
