...
var HOURS = {
  ...
  days: {
    0: [6.5, 18],                     // Sunday      6:30 AM – 6:00 PM
    1: [5.5, 18],                     // Monday      5:30 AM – 6:00 PM
    2: [5.5, 18],                     // Tuesday
    3: [5.5, 18],                     // Wednesday
    4: [5.5, 18],                     // Thursday
    5: [5.5, 18],                     // Friday
    6: [6.5, 18]                      // Saturday    6:30 AM – 6:00 PM
  }
};
...
(function(){
  ...
  /* ---------- SIGNATURE GADGET — THE FLAVOR WALL ----------
     Their real printed list, tapped out. One tap on a card rolls the real
     names out in place and flips the chevron (ARROW LAW). "Pick one for me"
     opens the syrup card and lights ONE real syrup — object + number, no
     sentence read back at the visitor. Tap it again for another. */
  $$('[data-roll]').forEach(function(roll){
    var btn = $('.roll-btn', roll);
    if(!btn) return;
    btn.addEventListener('click', function(){
      var open = roll.classList.toggle('open');
