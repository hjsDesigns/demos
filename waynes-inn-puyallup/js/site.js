...
var HOURS = {
  ...
  days: {                             // closes 2 AM every night -> 26
    0: [9, 26],                       // Sunday      9:00 AM - 2:00 AM
    1: [15, 26],                      // Monday      3:00 PM - 2:00 AM
    2: [11, 26],                      // Tuesday    11:00 AM - 2:00 AM
    3: [11, 26],                      // Wednesday  11:00 AM - 2:00 AM
    4: [11, 26],                      // Thursday   11:00 AM - 2:00 AM
    5: [11, 26],                      // Friday     11:00 AM - 2:00 AM
    6: [9, 26]                        // Saturday    9:00 AM - 2:00 AM
  }
};
/* Source: Restaurantji listing for Wayne's Inn Bar & Grill (visible hours block
   AND its schema.org openingHours, updated Aug 17 2026), matching the Google/Yelp
   Sunday 9 AM - 2 AM read taken during recon on 2026-09-06.
   HOLIDAY HOURS ARE UNCONFIRMED - no source publishes a Labor Day exception, so
   Monday shows its regular 3 PM open. */
...
(function(){
  ...
  /* ---------- SIGNATURE GADGET — THE READER BOARD ----------
     Their pole sign has a hand-set changeable-letter board (harvest g11 shows
     it reading FOOTBALL HERE). This is that board and it answers exactly one
     question, off the same Pacific clock the hours band uses: is karaoke
     tonight? The only recurring night Wayne's has a SOURCE for is the banner
     over their stage — "EVERY FRIDAY & SATURDAY NIGHT" — so those are the only
     two nights the board can ever claim. Sunday through Thursday, the next
     karaoke night is always Friday, so the second line is deterministic:
       Fri (5) / Sat (6) -> KARAOKE / TONIGHT
       every other day   -> KARAOKE / FRIDAY
     No hours, no invented events, no cycling states. One tap = lamps on and
     the letters clack into their slots; tap again = lamps off. */
  var board=$('#readerBoard');
  if(board){
    var l1=$('#rbLine1'), l2=$('#rbLine2');
    var reduceMo=window.matchMedia('(prefers-reduced-motion:reduce)').matches;
    function boardLines(){
      var d=pacificNow().day;
      return ['KARAOKE',(d===5||d===6)?'TONIGHT':'FRIDAY'];
    }
    function setLine(el,text,delay0){
      el.textContent='';
      var frag=document.createDocumentFragment();
      for(var i=0;i<text.length;i++){
        var sp=document.createElement('span');
        if(text.charAt(i)===' '){sp.className='sp';sp.innerHTML='&nbsp;'}
        else{sp.textContent=text.charAt(i)}
        frag.appendChild(sp);
      }
      el.appendChild(frag);
      var kids=Array.prototype.slice.call(el.children);
      kids.forEach(function(sp,i){
        if(reduceMo){sp.classList.add('set');return}
        setTimeout(function(){sp.classList.add('set')}, delay0 + i*55);
      });
    }
    function paintBoard(){
      var t=boardLines();
      setLine(l1,t[0],0);
      setLine(l2,t[1],t[0].length*55+90);
    }
    // On load the board is dark but the letters are already set — the sign is
    // readable before anyone touches it; tapping only turns the lamps on.
    var wasReduced=reduceMo; reduceMo=true; paintBoard(); reduceMo=wasReduced;
    board.addEventListener('click',function(){
      var lit=board.classList.toggle('lit');
      board.setAttribute('aria-pressed',lit?'true':'false');
      board.setAttribute('aria-label',lit?"Turn the reader board's lamps off":"Turn the reader board's lamps on");
      if(lit) paintBoard();
    });
    // keep the board honest if the page is left open across midnight
    setInterval(function(){
      var t=boardLines();
      if(l2.textContent.replace(/\u00a0/g,' ').trim()!==t[1]){
        var wr=reduceMo; reduceMo=!board.classList.contains('lit'); paintBoard(); reduceMo=wr;
      }
    },60000);
  }
