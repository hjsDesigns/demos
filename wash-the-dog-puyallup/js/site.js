...
var HOURS = {
  ...
  days: {
    // Sourced 2026-09-06 from the Yelp listing (mirrored on Yahoo Local) and
    // corroborated by scrubby.app, which mirrors the Google listing. Both agree.
    0: [10, 17],                      // Sunday      10:00 am - 5:00 pm
    1: [11, 17],                      // Monday      11:00 am - 5:00 pm
    2: null,                          // Tuesday     closed
    3: [11, 17],                      // Wednesday   11:00 am - 5:00 pm
    4: [11, 17],                      // Thursday    11:00 am - 5:00 pm
    5: [11, 17],                      // Friday      11:00 am - 5:00 pm
    6: [10, 17]                       // Saturday    10:00 am - 5:00 pm
  }
};
...
(function(){
  ...
  /* ---------- SIGNATURE GADGET — the seven steps ----------
     Their own sign, word for word. ONE button, one payoff: the wash runs
     down the ladder and each step lights in turn (Toddler Law — nothing to
     read before you can play with it, nothing to decode after). Tapping
     again restarts it. Reduced motion lights all seven at once. */
  (function(){
    var list=$('#washSteps'), go=$('#washGo');
    if(!list||!go) return;
    var rows=$$('li',list), timers=[], STEP=520;
    function clear(){timers.forEach(clearTimeout);timers=[];rows.forEach(function(r){r.classList.remove('on','done')})}
    function run(){
      clear();
      go.textContent='Washing\u2026';
      var reduce=window.matchMedia('(prefers-reduced-motion:reduce)').matches;
      if(reduce){rows.forEach(function(r){r.classList.add('done')});go.textContent='Run it again \u25b8';return}
      rows.forEach(function(row,i){
        timers.push(setTimeout(function(){
          if(i>0){rows[i-1].classList.remove('on');rows[i-1].classList.add('done')}
          row.classList.add('on');
        }, i*STEP));
      });
      timers.push(setTimeout(function(){
        rows[rows.length-1].classList.remove('on');
        rows[rows.length-1].classList.add('done');
