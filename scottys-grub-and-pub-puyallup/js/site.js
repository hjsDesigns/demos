/* ============================================================
   SCOTTY'S GRUB AND PUB — site.js
   Nav toggle · live open/closed clock (Pacific) · scroll-reveal ·
   count-up · contact form · title-cards-unroll · PULL A TAB (signature)
   ============================================================ */

/* ------------------------------------------------------------
   HOURS — Restaurantji's listing for Scotty's Grub and Pub
   (updated Sep 02, 2026) and the Google Maps place page checked
   2026-09-06 agree: 10 AM – 2 AM, all seven days.
   0 = Sunday … 6 = Saturday. Decimal 24h. 26 = 2 AM the next
   morning — the clock knows the bar is still open after midnight.
   ------------------------------------------------------------ */
var HOURS = {
  tz: 'America/Los_Angeles',
  days: {
    0: [10, 26],                      // Sunday    10:00 AM – 2:00 AM
    1: [10, 26],                      // Monday
    2: [10, 26],                      // Tuesday
    3: [10, 26],                      // Wednesday
    4: [10, 26],                      // Thursday
...
/* No holiday rules are published for this bar — the page says so in the
   hours note rather than guessing at one. Leave these as pass-throughs. */
...
(function(){
  ...
  /* ---------- SIGNATURE GADGET — PULL A TAB ------------------------
     Their awning says PULL TABS. One tap peels a tab open and shows one
     thing that's actually in the room; tap the same tab again and it
     shuts. Nothing is won, nothing is scored, nothing to read first. */
  var tabWrap=$('#tabs');
  if(tabWrap){
    var tabs=$$('.tab',tabWrap), said=$('#tabsSaid'), hint=$('#tabsHint'), opened=0;
    tabs.forEach(function(t){
      t.addEventListener('click',function(){
        var isOpen=t.getAttribute('aria-pressed')==='true';
        t.setAttribute('aria-pressed',isOpen?'false':'true');
        if(isOpen){
          said.textContent='';
        }else{
          said.textContent=t.getAttribute('data-said')||'';
          opened++;
          if(opened===1) hint.textContent='Tap it again to shut it.';
          if(opened>=3) hint.textContent='Keep going — there are five.';
        }
      });
    });
  }
