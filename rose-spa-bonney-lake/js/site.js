...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
/* ROSE SPA — 9:30 am to 10 pm, seven days (Google listing, checked 2026-09-05).
   Note for the polish pass: Fresha and Birdeye both list a 10:00 am open.
   The 9:30 here follows the live Google profile; confirm with the owner. */
var HOURS = {
...
(function(){
  ...
  if(form&&ok){form.addEventListener('submit',function(e){
    ...
    if(!key){ok.textContent='Preview only — no message was sent. Please call (253) 447-8676 to arrange an appointment.';done();return}
  ...
  /* ---------- TITLE CARDS UNROLL ----------
     Each service headline is its own tap target; the panel animates from 0 to
     its measured height, then to auto so it reflows on resize. Chevron flips. */
  $$('.roll-btn').forEach(function(btn){
    ...
    btn.addEventListener('click',function(){
      ...
      panel.setAttribute('aria-hidden',open?'false':'true'); panel.inert=!open;
      ...
      if(open){
        ...
        panel.style.height=panel.scrollHeight+'px';
        requestAnimationFrame(function(){requestAnimationFrame(function(){panel.style.height='0'})});
      }
    });
  });

  /* ---------- SIGNATURE GADGET — WHERE DOES IT HURT ----------
     Tap a part of the body, get the appointment that covers it. One tap in,
     one answer out, tap the same spot again to clear. Nothing typed, no
     prices and no hours in here — those live in their own sections. */
  var HURT={
    shoulders:{name:'Body massage',        dur:'30 or 60 minutes', line:'Say where it’s tight when you call — the therapist sets the pressure and stays on that area.'},
    back:     {name:'Body massage, deep pressure', dur:'60 minutes', line:'Deep pressure is one of the add-ons on the board at the front desk. Ask for it on the phone.'},
    feet:     {name:'Foot reflexology',    dur:'30 or 60 minutes', line:'Half an hour or a full hour on the feet, on its own or straight after a body massage.'},
    all:      {name:'Body and foot',       dur:'30 minutes each',  line:'Half the appointment on the body, half on the feet, in one room.'}
  };
  var hurtCard=$('#hurtCard'), hurtIdle=$('#hurtIdle'),
      hurtName=$('#hurtName'), hurtDur=$('#hurtDur'), hurtLine=$('#hurtLine');
  var hurtZones=$$('.hurt-zone'), hurtCurrent=null;
  function setHurt(key){
    hurtCurrent=key;
    hurtZones.forEach(function(z){
      var on=z.getAttribute('data-hurt')===key;
      z.classList.toggle('on',on);
      z.setAttribute('aria-pressed',on?'true':'false');
    });
    if(!key){hurtCard.classList.remove('show');hurtIdle.style.display='';return}
    var d=HURT[key]; if(!d)return;
    hurtName.textContent=d.name; hurtDur.textContent=d.dur; hurtLine.textContent=d.line;
  ...
  hurtZones.forEach(function(z){
    ...
    z.addEventListener('keydown',function(e){
      if(e.key==='Enter'||e.key===' '||e.key==='Spacebar'){e.preventDefault();hit()}
    });
  });
