...
var HOURS = {
  ...
  // Puget Construction Services — Google Business Profile, checked 2026-08-31.
  // (Yelp lists 7:00–3:30 for the same weekdays; Google's set ships, the
  //  30-minute variance is an open question for the owner.)
  days: {
    0: null,                          // Sunday      — closed
    1: [7.5, 16],                     // Monday      — 7:30 AM – 4:00 PM
    2: [7.5, 16],                     // Tuesday
    3: [7.5, 16],                     // Wednesday
    4: [7.5, 16],                     // Thursday
    5: [7.5, 16],                     // Friday
...
(function(){
  ...
  /* ---------- THE STREET SECTION ----------
     Four buttons, one drawn street. Tap what's happening on the surface and
     the cutaway shows what rolls up — each answer is one of the five real
     service lines. One tap, one visible change, no readouts, no hidden modes. */
  (function(){
    var stage = $('#streetStage'), cap = $('#streetCaption'), picks = $$('.pick');
    if(!stage || !cap || !picks.length) return;

    var IDLE = 'Four things happen on a street. Pick one.';
    var LINES = {
      vac:     'The vac truck parks over the manhole, water cuts the blockage loose and the tank hauls it away.',
      cctv:    'A camera crawls the whole line on video, so you see the crack or the root instead of guessing.',
      traffic: 'Cones, signs, arrow boards and flaggers, so traffic keeps moving around the hole in the road.',
      grout:   'Grout goes in under pressure, fills the void beneath the slab, and the surface comes back level.'
    };

    function set(state){
      stage.setAttribute('data-state', state || 'idle');
      cap.textContent = (state && LINES[state]) || IDLE;
    ...
    picks.forEach(function(b){
      b.addEventListener('click', function(){
        ...
        // tap it, it lights up; tap the same one again, it goes back to the quiet street
        set(stage.getAttribute('data-state') === s ? null : s);
      });
    });
