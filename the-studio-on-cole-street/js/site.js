...
var HOURS = {
  ...
  // The Studio on Cole Street — the self-serve BOOKING WINDOW (Google Business
  // Profile, confirmed 2026-09-04). There is no staffed front desk; access is
  // by reservation, so "open" here means "the room can be booked and entered".
  days: {
    0: [7, 22],                       // Sunday     7:00 am – 10:00 pm
    1: [7, 22],                       // Monday     7:00 am – 10:00 pm
    2: [7, 22],                       // Tuesday    7:00 am – 10:00 pm
    3: [7, 22],                       // Wednesday  7:00 am – 10:00 pm
    4: [7, 22],                       // Thursday   7:00 am – 10:00 pm
    5: [7, 22],                       // Friday     7:00 am – 10:00 pm
    6: [7, 21]                        // Saturday   7:00 am –  9:00 pm
  }
};
...
(function(){
  ...
  /* ---------- IN THE ROOM — tappable title cards (chevron unroll) ---------- */
  $$('.kit-head').forEach(function(btn){
    var card=btn.parentNode, body=document.getElementById(btn.getAttribute('aria-controls'));
    if(!body)return;
  ...
  /* ---------- SIGNATURE GADGET — THE SHUTTER ----------
     One big button. Each press flashes the frame and lands the next real
     photograph taken in this room. Three photos, all from the studio's own
     Instagram; captions describe exactly what is in the frame. */
  (function(){
    var wrap=$('#shutter'), btn=$('#shutterBtn'), snap=$('#snap'),
        img=$('#snapImg'), cap=$('#snapCap'), count=$('#shutterCount');
    if(!wrap||!btn||!snap||!img||!cap||!count)return;

    var ROLL=[
      {src:'images/print-1.jpg',
       cap:'Camera in hand, on the drape',
       alt:'A photographer standing with her camera in front of the studio’s cream draped backdrop'},
      {src:'images/print-2.jpg',
       cap:'A first birthday, in the chair',
       alt:'A one-year-old in a party hat kneeling on the studio’s rattan peacock chair beside a bunch of white balloons'},
      {src:'images/print-3.jpg',
       cap:'The holiday set, built in the room',
       alt:'The studio’s holiday set — a lit tree with ribbon bows behind the tan leather sofa'}
    ];
    // preload so a press never shows an empty frame
    ROLL.forEach(function(f){var p=new Image();p.src=f.src});
    ...
    function show(n){
      var f=ROLL[n];
      img.src=f.src; img.alt=f.alt; cap.textContent=f.cap;
      count.textContent=pad(n+1)+' / '+pad(ROLL.length);
    }
    function fire(){
      if(busy)return;
      i=(i+1)%ROLL.length;
      if(reduce.matches){show(i);return}
      busy=true;
      snap.classList.add('busy'); wrap.classList.add('firing');
      setTimeout(function(){show(i)},150);
      setTimeout(function(){
        snap.classList.remove('busy'); wrap.classList.remove('firing'); busy=false;
      },430);
    }
