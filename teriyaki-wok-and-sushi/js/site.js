...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
/* Teriyaki Wok & Sushi, Bonney Lake.
   Sun / Tue–Fri / Sat from their own site (teriyakiwoksushiwa.com, "BUSINESS
   HOURS"). Monday is omitted there; Restaurantji's listing for the same
   address + phone says Monday closed, so that's what ships. */
var HOURS = {
  tz: 'America/Los_Angeles',          // Pacific, wherever the viewer is
  days: {
    0: [12, 20.5],                    // Sunday      12:00 pm – 8:30 pm
    1: null,                          // Monday      closed
    2: [11, 20.5],                    // Tuesday     11:00 am – 8:30 pm
    3: [11, 20.5],                    // Wednesday   11:00 am – 8:30 pm
    4: [11, 20.5],                    // Thursday    11:00 am – 8:30 pm
    5: [11, 20.5],                    // Friday      11:00 am – 8:30 pm
    6: [12, 20.5]                     // Saturday    12:00 pm – 8:30 pm
  }
};
...
(function(){
  ...
  if(form&&ok){form.addEventListener('submit',function(e){
    ...
    if(!key){ok.textContent='Preview only — no message was sent. Call (253) 891-1980 or use Order online to place an order.';done();return}
  ...
  /* ---------- MENU TITLE CARDS ----------
     Each category headline is itself the tap target and rolls the real items
     out in place (TITLE CARDS UNROLL). The affordance is a downward chevron
     that flips up (ARROW LAW) — never a plus sign. */
  $$('.card-roll').forEach(function(card,index){
    var head=$('.roll-head',card), body=$('.roll-body',card);
    if(!head||!body)return;
    body.id='menu-category-'+index; head.setAttribute('aria-controls',body.id); body.hidden=true;
  ...
  /* ---------- SIGNATURE GADGET — BUILD YOUR BENTO ----------
     Five real bentos off their own ordering page (B-3 … B-7). Every box
     carries the same four sides, so the only choice is the main: tap a pill,
     the photo and the price change. No numbers to type, no modes, no legend.
     `note` is set only where the photo shows the main plated rather than
     boxed — so nothing on screen claims to be a picture it isn't. */
  var BENTO={
    chicken : {name:'B-3 Chicken Teriyaki Bento', price:'$19.95', note:''},
    beef    : {name:'B-4 Beef Teriyaki Bento',    price:'$20.95', note:'Photo shows their beef teriyaki, the main in this box.'},
    pork    : {name:'B-5 Pork Teriyaki Bento',    price:'$19.95', note:'Photo shows their pork teriyaki, the main in this box.'},
    salmon  : {name:'B-6 Salmon Teri Bento',      price:'$20.95', note:''},
    shortrib: {name:'B-7 Short Rib Bento',        price:'$28.95', note:'Photo shows their beef short ribs, the main in this box.'}
  };
  var picks=$$('.bento-picks .pick'), shots=$$('.bento-shot .bshot'),
      bName=$('#bentoName'), bPrice=$('#bentoPrice'), bNote=$('#bentoNote');
  if(picks.length && bName && bPrice){
    var setBento=function(k){
      var d=BENTO[k]; if(!d)return;
      picks.forEach(function(b){var on=b.getAttribute('data-k')===k;
        b.classList.toggle('is-on',on); b.setAttribute('aria-pressed',on?'true':'false')});
      shots.forEach(function(im){var active=im.getAttribute('data-k')===k;im.classList.toggle('on',active);im.setAttribute('aria-hidden',active?'false':'true')});
      bName.textContent=d.name; bPrice.textContent=d.price;
      if(d.note){bNote.textContent=d.note;bNote.hidden=false}else{bNote.textContent='';bNote.hidden=true}
    };
    picks.forEach(function(b){b.addEventListener('click',function(){setBento(b.getAttribute('data-k'))})});
    setBento('chicken');
  }

  /* Search existing dishes without replacing the browsable category cards. */
  var search=$('#menuSearch'), clearSearch=$('#clearMenuSearch'), searchStatus=$('#menuSearchStatus'), bentoLink=$('#bentoSearchLink');
  if(search){
    $('.menu-search').hidden=false;
    function filterMenu(){
      var query=search.value.trim().toLocaleLowerCase(), count=0;
      $$('.card-roll').forEach(function(card){
        var matches=0, head=$('.roll-head',card), body=$('.roll-body',card);
        $$('.menu-list li',card).forEach(function(row){
          var match=!query||row.textContent.toLocaleLowerCase().indexOf(query)!==-1;
          row.hidden=!match;if(match)matches++;
        });
        card.hidden=!!query&&!matches;count+=matches;
        var open=!!query&&!!matches;card.classList.toggle('is-open',open);
        head.setAttribute('aria-expanded',open?'true':'false');body.hidden=!open;
      });
      var bentos=query?Object.keys(BENTO).filter(function(k){return BENTO[k].name.toLocaleLowerCase().indexOf(query)!==-1}):[];
      bentoLink.hidden=!bentos.length;bentoLink.setAttribute('data-match',bentos[0]||'');clearSearch.hidden=!query;
      searchStatus.textContent=!query?'':count+' menu '+(count===1?'match':'matches')+(bentos.length?' + '+bentos.length+' bento '+(bentos.length===1?'box':'boxes'):'')+(count||bentos.length?'':' — try another dish.');
    }
    bentoLink.addEventListener('click',function(){var match=bentoLink.getAttribute('data-match');if(match)setBento(match)});
    search.addEventListener('input',filterMenu);
    clearSearch.addEventListener('click',function(){search.value='';filterMenu();search.focus()});
  }
})();
