/* PNW ADU — hero "Day into sunset": the add-on farmhouse (Hayden's Higgsfield concept render — "that house is
   ...
   Three jobs here:
     ...
     2. THE SWEEP — one GSAP timeline, ~3.5 s, deterministic, once (SWEEP below). Two numbers drive it:
...
(function () {
  ...
  /* Timing in seconds: .25 hold + .7 dusk delay + 2.55 dusk = 3.5 total.
     The sunset grade rises and falls within that span. The title animates independently
     in CSS from first paint, so reading is never gated on image decoding or dusk. */
  var SWEEP = { hold: .25, duskDelay: .7, dusk: 2.55, goldUp: 1.2, goldHold: .5, goldDown: 1.3, peak: .55 }; // 3.5 seconds; copy appears independently from first paint.
  ...
  /* Wait for both gates in either order: no cutout request competes with the initial page load or the 3.5 s sweep.
  ...
  function runSweep() {
    ...
    /* start only once both stills are decoded — the .25 s hold is on the DAY house, not on a blank box.
