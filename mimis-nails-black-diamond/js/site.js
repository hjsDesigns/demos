...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
var HOURS = {tz: 'America/Los_Angeles',days:{0:[10,15],1:[9.5,19],2:[9.5,19],3:[9.5,19],4:[9.5,19],5:[9.5,19],6:[9,18]}};
...
(function(){
  ...
  function setNav(open){nav.classList.toggle('open',open);toggle.setAttribute('aria-expanded',open?'true':'false');toggle.setAttribute('aria-label',open?'Close navigation':'Open navigation');toggle.textContent=open?'✕':'☰'}
  if(toggle&&nav){
  ...
  /* Actual-work lookbook: one tap filters, one tap enlarges. */
  var looks=$$('.look'), filters=$$('.look-filters button'), count=$('#look-count');
  filters.forEach(function(button){button.addEventListener('click',function(){
    var category=button.dataset.filter,visible=0;
    filters.forEach(function(other){other.setAttribute('aria-pressed',String(other===button))});
    looks.forEach(function(look){look.hidden=category!=='all'&&look.dataset.category!==category;if(!look.hidden)visible++});
    count.textContent=visible+' looks';
  })});
  var dialog=$('#photo-dialog'), dialogImage=$('#dialog-image'), caption=$('#dialog-caption'), previousFocus=null;
  $$('.look-open').forEach(function(button){button.addEventListener('click',function(event){event.preventDefault();
    var source=$('img',button);previousFocus=button;
    var expanded=new Image();expanded.src=source.getAttribute('src');expanded.alt=source.alt;dialogImage.replaceChildren(expanded);caption.textContent=button.dataset.caption;
  ...
  dialog.addEventListener('close',function(){document.body.classList.remove('dialog-open');if(previousFocus)previousFocus.focus({preventScroll:true})});
