...
(function(){
  ...
  var io=new IntersectionObserver(function(entries){
    entries.forEach(function(en){
      ...
      $$('.count',en.target).forEach(function(c){
        ...
        var to=parseFloat(c.getAttribute('data-to')), from=parseFloat(c.getAttribute('data-from')||'0'), t0=null;
        var show=function(v){return dec?v.toFixed(dec):String(Math.round(v))};
        var reduce=window.matchMedia('(prefers-reduced-motion:reduce)').matches;
        if(reduce){c.textContent=show(to);return}
        function step(ts){if(!t0)t0=ts;var k=Math.min(1,(ts-t0)/1400);var e=1-Math.pow(1-k,3);c.textContent=show(from+(to-from)*e);if(k<1)requestAnimationFrame(step)}
  ...
  /* ---------- THE BOARD: title cards roll their list out (chevrons, never a plus) ---------- */
  $$('.roll-head').forEach(function(btn){
    btn.addEventListener('click',function(){
      var roll=btn.closest('.roll'), open=!roll.classList.contains('open');
      roll.classList.toggle('open',open);
      btn.setAttribute('aria-expanded',open?'true':'false');
    });
  });

  /* ---------- SIGNATURE GADGET — THE CREAMERY ----------
     Their real Ice Cream board, turned into one tap. Prices are transcribed
     straight off the sign bolted beside the window:
       Soft Serve Cone/Cup  3.50 / 4.00 / 4.50   (S / M / L)
       Affogato             4.75 / 5.25 / 5.75
       Ice Cream Float      4.50 / 5.00 / 5.50
       Hot Fudge Sundae     4.50 / 5.00 / 5.50
       Extras: Ice Cream Toppings .50  ·  Waffle Cone .50
     Output is the OBJECT and the NUMBER — no sentence narrating the choice back. */
  (function(){
    var art=$('#crArt'), amt=$('#crPrice'), what=$('#crWhat'), topBtn=$('#crTop');
    if(!art||!amt||!what||!topBtn)return;

    var ITEMS={
      cup:     {base:[3.50,4.00,4.50], vessel:'vCup',   swirl:true,  label:'soft serve, in a cup',   cls:''},
      cone:    {base:[3.50,4.00,4.50], vessel:'vCone',  swirl:true,  label:'soft serve, waffle cone',cls:'', waffle:true},
      affogato:{base:[4.75,5.25,5.75], vessel:'vAffo',  swirl:false, label:'affogato',               cls:'affo no-swirl'},
      float:   {base:[4.50,5.00,5.50], vessel:'vGlass', swirl:true,  label:'ice cream float',        cls:'float'},
      sundae:  {base:[4.50,5.00,5.50], vessel:'vCup',   swirl:true,  label:'hot fudge sundae',       cls:'fudge'}
    };
    var SIZES={s:{i:0,name:'Small'},m:{i:1,name:'Medium'},l:{i:2,name:'Large'}};
    var WAFFLE=0.50, TOPPINGS=0.50;
    var item='cup', size='m', tops=false;

    function render(){
      var it=ITEMS[item], si=SIZES[size];
      var p=it.base[si.i] + (it.waffle?WAFFLE:0) + (tops?TOPPINGS:0);
      amt.textContent=p.toFixed(2);
      what.textContent=si.name+' '+it.label;
      art.setAttribute('class','cr-art'+(it.cls?' '+it.cls:'')+(tops&&it.swirl?' tops':''));
      art.setAttribute('data-size',size);
      $$('.cr-vessel',art).forEach(function(g){g.classList.toggle('on',g.id===it.vessel)});
    }

    $$('.cr-pick').forEach(function(b){
      b.addEventListener('click',function(){
        item=b.getAttribute('data-item');
        $$('.cr-pick').forEach(function(o){o.classList.toggle('is-on',o===b)});
        render();
      });
    });
    $$('.cr-size').forEach(function(b){
      b.addEventListener('click',function(){
        size=b.getAttribute('data-size');
        $$('.cr-size').forEach(function(o){o.classList.toggle('is-on',o===b)});
        render();
      });
    });
    topBtn.addEventListener('click',function(){
      tops=!tops; topBtn.setAttribute('aria-pressed',tops?'true':'false'); render();
    });
