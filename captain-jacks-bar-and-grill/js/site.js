/* ============================================================
   CAPTAIN JACK'S BAR & GRILL — site.js
   Nav toggle · live open/closed clock (Pacific) · scroll-reveal ·
   count-up · contact form · title-cards-unroll · THE TANK (goldfish races)
   ============================================================ */

/* ------------------------------------------------------------
   HOURS — from the Yelp listing ("Updated August 2026") and
   Restaurantji (updated Jul 12, 2026), which agree exactly.
   0 = Sunday … 6 = Saturday. Decimal 24h. 26 = 2 AM the next morning,
   24 = midnight — the clock knows the bar is still open after midnight.
   ------------------------------------------------------------ */
var HOURS = {
...
/* No holiday or seasonal rules confirmed for this bar — leave as-is. */
...
(function(){
  ...
  /* ---------- SIGNATURE GADGET — THE TANK -------------------------
     Captain Jack's runs real goldfish races (their own "PICK YOUR FISH"
     flyers; flag drops at 7:00 PM). One tap on a fish = your fish, and
     all five run. No prize, no stake — just the race. */
  var tank=$('#tank');
  if(tank){
    var fishes=$$('.fish',tank), result=$('#tankResult'), hint=$('#tankHint'), running=false;

    function runDistance(){
      var lane=tank.querySelector('.lane'), f=fishes[0];
      // finish line sits at right:16px and is 9px wide
      return Math.max(40, lane.clientWidth - 25 - f.offsetLeft - f.offsetWidth);
    }

    function settle(winner, picked){
      running=false;
      result.textContent = (winner.lane===picked)
        ? 'Your fish took it — lane ' + winner.lane + '.'
        : 'Lane ' + winner.lane + ' touched the wall first.';
      hint.textContent = 'Tap another fish to run it again.';
    }

    function race(picked){
      running=true;
      var d=runDistance(), reduce=REDUCE(), runs=[];
      fishes.forEach(function(f){
        f.classList.remove('swim');
        f.style.transform='';
        var t = 2.3 + Math.random()*1.5;
        runs.push({lane:f.getAttribute('data-lane'), t:t});
        f.style.setProperty('--dist', d+'px');
        f.style.setProperty('--dur', t.toFixed(2)+'s');
      });
      void tank.offsetWidth;                       // reflow so the animation restarts cleanly
      var winner=runs.slice().sort(function(a,b){return a.t-b.t})[0];
      var last=runs.reduce(function(m,x){return Math.max(m,x.t)},0);
      if(reduce){
        fishes.forEach(function(f){f.style.transform='translateX('+d+'px)'});
        settle(winner,picked);
        return;
      }
      fishes.forEach(function(f){f.classList.add('swim')});
      setTimeout(function(){settle(winner,picked)}, last*1000+80);
    }

    fishes.forEach(function(f){
      f.addEventListener('click',function(){
        if(running) return;
        var lane=f.getAttribute('data-lane');
        fishes.forEach(function(o){o.setAttribute('aria-pressed', o===f?'true':'false')});
        result.textContent='';
        race(lane);
      });
    });
