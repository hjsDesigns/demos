...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
/* THE CAST IRON QUEEN — published weekly hours.
   Source: restaurantguru.com/The-Cast-Iron-Queen-Enumclaw (schema.org openingHours
   "Fr 16:00-20:00 / Sa 09:00-15:00 / Su 09:00-15:00" AND the day table), corroborated
   by maps.roadtrippers.com. Both fetched 2026-09-05. Owner still to confirm. */
var HOURS = {
  tz: 'America/Los_Angeles',          // Pacific, wherever the viewer is
  days: {
    0: [9, 15],                       // Sunday     9 am – 3 pm
    1: null,                          // Monday     closed
    2: null,                          // Tuesday    closed
    3: null,                          // Wednesday  closed
    4: null,                          // Thursday   closed
    5: [16, 20],                      // Friday     4 pm – 8 pm
    6: [9, 15]                        // Saturday   9 am – 3 pm
  }
};
...
(function(){
  ...
  if(form&&ok){
    ...
    btn.disabled=false;
    if(keyEl&&keyEl.value.trim()){btn.textContent='Send question';help.textContent='Your question goes to the configured truck inbox.';}
    ...
    form.addEventListener('submit',function(e){
      ...
      if(!key){
        ...
        function selectNote(){area.focus();area.select();feedback('Your note is ready below. Select and copy it for your conversation with the truck; nothing has been sent.',false);}
        if(navigator.clipboard&&navigator.clipboard.writeText){
          navigator.clipboard.writeText(body).then(function(){feedback('Question copied. Keep it handy when you call the truck; nothing has been sent.',false);}).catch(selectNote);
        }else selectNote();
        return;
      }
      var label=btn.textContent;btn.disabled=true;btn.textContent='Sending…';
      var data={};new FormData(form).forEach(function(v,k){data[k]=v});data.subject='Question — The Cast Iron Queen';
      fetch('https://api.web3forms.com/submit',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify(data)})
        ...
        .then(function(result){if(!result||!result.success)throw new Error('send failed');feedback('Message sent. The truck can reply using the contact details you provided.',false);btn.textContent='Message sent';})
        .catch(function(){btn.disabled=false;btn.textContent=label;feedback('Could not send just now. Call (253) 740-4538 instead.',true);});
    });
  }
  ...
  /* ---------- SIGNATURE GADGET: "what are you hungry for" ----------
     ...
     read; tapping the one that is already showing simply leaves it showing. */
  (function(){
    var picks=$$('.hungry-pick'), plate=$('#hungryPlate'),
        nameEl=$('#hungryName'), descEl=$('#hungryDesc');
    if(!picks.length||!plate||!nameEl||!descEl) return;
    function show(k,name,desc){
      $$('img',plate).forEach(function(im){var selected=im.getAttribute('data-k')===k;im.classList.toggle('on',selected);im.setAttribute('aria-hidden',String(!selected||im.classList.contains('echo')))});
      picks.forEach(function(b){b.setAttribute('aria-pressed',b.getAttribute('data-k')===k?'true':'false')});
      nameEl.textContent=name; descEl.textContent=desc;
    }
    picks.forEach(function(b){
      b.disabled=false;
      b.addEventListener('click',function(){
        show(b.getAttribute('data-k'),b.getAttribute('data-name'),b.getAttribute('data-desc'));
      });
    });
  })();
