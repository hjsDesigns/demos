/* ============================================================
   A SPOONFUL OF SUGAR — Milton, WA — site.js
   Nav toggle · live open/closed clock (Pacific) · scroll-reveal ·
   count-up · contact form · title-cards-unroll · THE BOARD (house drinks)
   ============================================================ */

/* ------------------------------------------------------------
   HOURS — Restaurantji listing for A Spoonful Of Sugar, 2420 Milton Way
   (page updated Sep 05, 2026): Mon–Fri 5 AM–3 PM, Sat 6:30 AM–3 PM,
   Sun 7:30 AM–3 PM. Google Maps agrees on the 5 AM weekday open.
   NOTE: their own printed to-go menu reads "M-F 5AM-5PM" — that print is
   older than the listing; the 3 PM close is what's carried here and it is
   on the ask-client list in HANDOFF-CODEX.md.
   0 = Sunday … 6 = Saturday. Decimal 24h (6.5 = 6:30 AM).
   ------------------------------------------------------------ */
var HOURS = {
  tz: 'America/Los_Angeles',
  days: {
    0: [7.5, 15],                     // Sunday    7:30 AM – 3:00 PM
    1: [5, 15],                       // Monday    5:00 AM – 3:00 PM
    2: [5, 15],                       // Tuesday
    3: [5, 15],                       // Wednesday
    4: [5, 15],                       // Thursday
    5: [5, 15],                       // Friday
    6: [6.5, 15]                      // Saturday  6:30 AM – 3:00 PM
  }
};

/* No holiday rule is carried here: a "WE WILL BE CLOSED LABOR DAY" sign is
   visible on their counter in one of their own menu-board photos, but that
   photo is undated, so nothing about this year is confirmed. The hours band
   tells visitors to call on a holiday Monday instead of guessing for them. */
...
(function(){
  ...
  /* ---------- contact form (demo mode: access_key is empty, nothing sends) ---------- */
  ...
  if(form&&ok){form.addEventListener('submit',function(e){
    if(!form.querySelector('input[name=access_key]').value){
      e.preventDefault();ok.classList.add('show');form.querySelector('button[type=submit]').disabled=true;
    }
  })}
  ...
  /* ---------- SIGNATURE — THE BOARD -------------------------------
     The five house drinks printed across the top of their own menu
     board. One tap on a cup: the cup fills and the real recipe prints
     underneath. Tap the same cup again and it empties. No modes, no
     numbers, nothing to read before you start. */
  var row=$('#cupRow'), out=$('#cupOut');
  if(row&&out){
    var idle=out.innerHTML;
    var cups=$$('.cup-btn',row);
    cups.forEach(function(btn){
      btn.addEventListener('click',function(){
        var on=btn.getAttribute('aria-pressed')==='true';
        cups.forEach(function(o){o.setAttribute('aria-pressed','false')});
        if(on){ out.innerHTML=idle; return; }
        btn.setAttribute('aria-pressed','true');
        var n=document.createElement('p'); n.className='oname'; n.textContent=btn.getAttribute('data-name');
        var r=document.createElement('p'); r.className='orecipe'; r.textContent=btn.getAttribute('data-recipe');
        out.innerHTML=''; out.appendChild(n); out.appendChild(r);
      });
    });
  }
