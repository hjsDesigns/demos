...
var HOURS = {
  ...
  days: {                             // Sumner Nails — all 7 days, off the live Google panel
    0: [11, 15],                      // Sunday     11 AM – 3 PM
    1: [9, 17],                       // Monday      9 AM – 5 PM
    2: [9, 17],                       // Tuesday     9 AM – 5 PM
...
(function(){
  ...
  /* ---------- OFFERINGS: title cards unroll (ARROW LAW: chevron down, flips up) ---------- */
  $$('.unroll').forEach(function(card){
    var head=$('.unroll-head',card), body=$('.unroll-body',card);
    if(!head||!body) return;
  ...
  (function(){
    ...
    var shots=$$('.shot[data-sw]',frame), current=null;
    function show(key){
      current = key;
      shots.forEach(function(sh){sh.classList.toggle('on', !!key && sh.getAttribute('data-sw')===key)});
      pills.forEach(function(b){b.setAttribute('aria-pressed', b.getAttribute('data-sw')===key ? 'true':'false')});
      frame.classList.toggle('has', !!key);
      if(cap) cap.textContent = key ? (pills.filter(function(b){return b.getAttribute('data-sw')===key})[0]||{}).dataset.cap || '' : '';
    }
