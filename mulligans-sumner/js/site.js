/* ============================================================
   ...
   Nav toggle · live open/closed clock (Pacific time, with a third
   NEUTRAL state for unpublished hours) · scroll-reveal · count-up ·
   contact form · title-cards-unroll · TAKE A MULLIGAN (the tee strip).
...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
/* MULLIGANS, SUMNER — hours are genuinely UNPUBLISHED.
   The room changed hands in 2025 and the Mulligans group has not posted
   hours for it anywhere: mulligansgc.com carries only the Buckley club,
   the pub's Facebook has not been updated since Sept 2025, and Google
   still shows the previous operator's listing (stale — Hayden drove past
   on 6 Sept 2026 and the old business is gone).

   So the clock does NOT guess. `unpublished:true` puts the site into a
   third, NEUTRAL state — amber light, "call for hours" on all seven rows,
   no green "Open now" and no red "Closed" either, because both would be
   a lie. Delete the flag and fill `days` the moment the owner confirms
   real hours; everything else already works. */
var HOURS = {
  tz: 'America/Los_Angeles',          // Pacific, wherever the viewer is
  unpublished: true,
  unpublishedText: 'Hours aren’t posted yet — call before you go',
  days: {
    0: null,                          // Sunday      — unconfirmed
    1: null,                          // Monday      — unconfirmed
    2: null,                          // Tuesday     — unconfirmed
    3: null,                          // Wednesday   — unconfirmed
    4: null,                          // Thursday    — unconfirmed
    5: null,                          // Friday      — unconfirmed
    6: null                           // Saturday    — unconfirmed
  }
};
...
(function(){
  ...
  function computeStatus(){
    ...
    if(HOURS.unpublished) return {unknown:true,open:false,text:HOURS.unpublishedText};
  ...
  function applyStatus(){
    ...
    var state=s.unknown?'is-unknown':(s.open?'is-open':'is-closed');
    var hp=$('#hdrLive'), ht=$('#hdrLiveText');
    if(hp&&ht){hp.className='hdr-live '+state;
      ht.innerHTML=s.unknown?'Call for hours':((s.open?'Open':'Closed')+' <span class="txt-long">· '+s.text.replace(/^Open now · /,'').replace(/^Closed · /,'')+'</span>');}
    var sl=$('#statusLine'), st=$('#statusText');
    if(sl&&st){sl.className='live-line '+state;st.textContent=s.text;}
  ...
  /* ---------- TITLE CARDS UNROLL — the headline is the tap target ----------
     ARROW LAW: the affordance is a downward chevron that flips up. */
  ...
  /* ---------- SIGNATURE GADGET — TAKE A MULLIGAN -------------------
     Their name is the toy. A mulligan is the shot you get to hit again:
     one tap on the ball, it arcs down the strip and drops in the cup,
     the flag waves, the count above ticks up, the ball is back on the
     tee. No modes, no legend, nothing to read, nothing to win.

     DETERMINISTIC by design (APPROVED IS LOCKED): the four arc heights
  ...
  if(strip&&ball&&cup&&out){
    ...
    function land(){
      ...
      setTimeout(function(){
        ...
        ball.classList.remove('is-sunk','is-flying');
        flying=false;
      },380);
    }
    ...
    function swing(){
      if(flying) return;
      flying=true; shots++;
      var b=ball.getBoundingClientRect(), c=cup.getBoundingClientRect();
      var dx=(c.left+c.width/2)-(b.left+b.width/2);
      var dy=(c.top+c.height/2)-(b.top+b.height/2);
      /* Keep the whole arc inside the strip — .tee-strip clips, and a ball
         that vanishes off the top reads as a bug, not a shot. */
      var sr=strip.getBoundingClientRect();
      var headroom=(b.top-sr.top)-6;
      var peak=Math.min(sr.height*ARCS[(shots-1)%ARCS.length],Math.max(28,headroom));
      ...
      function step(ts){
        ...
        var x=dx*k;
        var y=dy*k - peak*4*k*(1-k);          /* parabola: up, over, in */
        ball.style.transform='translate('+x+'px,'+y+'px) rotate('+(k*540)+'deg)';
        if(k<1){raf=requestAnimationFrame(step)} else {land()}
      }
      raf=requestAnimationFrame(step);
    }

    ball.addEventListener('click',swing);
  }
