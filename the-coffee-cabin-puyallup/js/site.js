...
var HOURS = {
  ...
  days: {
    ...
    1: [4, 17],                       // Monday      4:00 am – 5:00 pm
    2: [4, 17],                       // Tuesday
    3: [4, 17],                       // Wednesday
    4: [4, 17],                       // Thursday
    5: [4, 17],                       // Friday
    6: [5, 17]                        // Saturday    5:00 am – 5:00 pm
  }
};
...
(function(){
  ...
  /* ---------- MENU: title cards unroll ----------
     TITLE CARDS UNROLL law — the category heading IS the tap target and rolls
     its real items out in place; the chevron flips up (ARROW LAW). Several can
     be open at once; "Hot drinks" ships open in the HTML so nothing shifts on
     load and the section is never a wall of closed doors. */
  $$('.roll-t').forEach(function(btn){
    btn.addEventListener('click',function(){
      var roll=btn.closest('.roll');
      if(!roll)return;
      var open=roll.classList.toggle('is-open');
      btn.setAttribute('aria-expanded',open?'true':'false');
    });
  });

  /* ---------- SIGNATURE GADGET — THE PUNCH CARD ----------
     Their real "Buy 10 Get 1 FREE" bear card, on screen.
     ONE gesture: tap a bear, it stamps. Tap a stamped bear, it clears.
     Ten stamps lights the free cup. No modes, no instructions, no numbers to
     type, and nothing about hours, prices or the address lives in here. */
  var punch=$('#punch'), bearsBox=$('#bears'), punchLabel=$('#punchLabel');
  if(punch&&bearsBox&&punchLabel){
    // The ten bears live in the HTML (see #bears) so the card holds its space
    // from first paint — injecting them here cost 0.14 of layout shift.
    var TOTAL=$$('.bear',bearsBox).length || 10;

    function stampedCount(){
      return $$('.bear',bearsBox).filter(function(b){return b.getAttribute('aria-pressed')==='true'}).length;
    }
    function render(){
      var n=stampedCount(), full=(n===TOTAL);
      punch.classList.toggle('is-full',full);
      punchLabel.textContent = full ? 'Free drink — earned' : 'Free drink';
    }
    bearsBox.addEventListener('click',function(e){
      var b=e.target.closest('.bear'); if(!b||!bearsBox.contains(b))return;
