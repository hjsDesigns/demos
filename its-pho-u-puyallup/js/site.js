...
var HOURS = {
  ...
  days: {
    ...
    1: [10.5, 21.5],                  // Monday      10:30 AM – 9:30 PM
    2: [10.5, 21.5],                  // Tuesday
    3: [10.5, 21.5],                  // Wednesday
    4: [10.5, 21.5],                  // Thursday
    5: [10.5, 21.5],                  // Friday
    6: [10.5, 21.5]                   // Saturday
  }
};
/* SOURCE: their own current to-go sheet, photographed in the dining room
   ("Business Hours: 10:30 am - 9:30 pm, Sundays Closed" — harvest g14), the
   Restaurantji listing updated 2026-08-24, and Google Maps 2026-09-06
   ("Opens 10:30 AM Mon"). The older laminated menu cover in the dining room
   still prints 10:00 pm — two newer sources beat it. See facts.json. */
...
(function(){
  ...
  /* ---------- SIGNATURE GADGET — DRESS YOUR BOWL ----------
     Their real side plate: Thai basil, bean sprouts, jalapeño, lime. Each
     button is a crop of that plate from their own photo. One tap drops it
     into the bowl at a FIXED landing spot (deterministic — a repeat tap
     always lands identically); tap again and it lifts back out. No readback
     sentence, no numbers, no hidden modes. */
  var plate=$('#garnishes'), drops=$('#drops');
  if(plate&&drops){
    $$('.garnish',plate).forEach(function(btn){
      btn.addEventListener('click',function(){
        var g=btn.getAttribute('data-g');
        var drop=drops.querySelector('.drop[data-g="'+g+'"]');
        var on=btn.getAttribute('aria-pressed')==='true';
        btn.setAttribute('aria-pressed', on?'false':'true');
        if(!drop) return;
        if(on){ drop.classList.remove('in'); }
        else { drop.classList.remove('in'); void drop.offsetWidth; drop.classList.add('in'); }
      });
    });
  }

  /* ---------- (template signature slot) ----------
